...
import { createLowPolyRenderer } from './lowPolyBust';
import { createScanRenderer } from './scanPortrait';
...
/*
 * Avatar do hero: retrato "3D scan" (src/assets/avatar-scan.webp — mapa de brilho em tons
 * de cinza, colorido em runtime por src/Avatar3D/scanPortrait.ts).
 *   entrada: linha de varredura desce revelando o retrato
 *   repouso: paralaxe 3D com o cursor, flutuação, faíscas e varredura a cada ~7s
 *   scroll:  vira partículas que sobem e esquentam para âmbar ("dissolve")
 *
 * Sem o arquivo, entra o busto low-poly gerado por código (lowPolyBust.ts).
 * Pausa fora da tela e fica estático com "menos movimento".
 */

const scanFiles = import.meta.glob<string>('/src/assets/avatar-scan.{webp,png,jpg,jpeg}', {
  eager: true,
  query: '?url',
  import: 'default',
});
const AVATAR_URL: string | undefined = Object.values(scanFiles)[0];
...
export function Avatar3D({ className }: Avatar3DProps) {
  ...
  useEffect(() => {
    ...
    let renderer: Renderer | null = AVATAR_URL ? null : createLowPolyRenderer();
    let isImage = false;
    let cancelled = false;
    ...
    const draw = (now: number) => {
      ...
      if (isImage && !reduced) {
        const rotY = pointer.x * 8 + s * 20;
        const rotX = -pointer.y * 5.5;
    ...
    if (AVATAR_URL) {
      ...
      img
        ...
        .then(() => {
          if (cancelled) return;
          renderer = createScanRenderer(img);
          isImage = true;
          if (!raf) draw(performance.now());
        })
        .catch(() => {
          if (cancelled) return;
          renderer = createLowPolyRenderer();
          if (!raf) draw(performance.now());
        });
    }
  ...
  return (
    <div
      ...
      aria-hidden="true"
      className={cn(
        'cut-corner relative isolate aspect-square w-full overflow-hidden bg-graphite shadow-lift',
        className,
      )}
    >
      ...
      {/* Moldura do conceito (●──● ╲╱ FACE ╱╲ ●──●) — só no busto low-poly:
          o retrato de scan já traz a própria marcação de HUD. */}
      {AVATAR_URL ? null : (
        <>
          <svg
            data-ring
      ...
      <span className="absolute top-5 left-6 font-mono text-[0.625rem] uppercase tracking-[0.16em]">
        <span className="text-cyber">01 //</span> <span className="text-ink-dim">Face</span>
      </span>
    </div>
  );
}
